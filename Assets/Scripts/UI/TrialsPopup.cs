...
{
    /// <summary>
    /// Trials popup: per-run challenge rows (best value achieved across runs,
    /// vs. target) cloned from <see cref="rowTemplate"/>. Mirrors
    /// <see cref="GaragePopup"/>'s pattern; reward claim uses the same gate as
    /// <see cref="MissionsPopup"/> (no silent auto-grant).
    /// </summary>
    public class TrialsPopup : Popup
    {
        ...
        void Populate()
        {
            ...
            {
                var def = defs[i];
                if (def == null || def.scope != MissionScope.PerRunTrial) continue;
